...
async function run() {
    try {
        const data = await fetchJSON('https://unsplash.com/napi/search/photos?query=indian%20business%20portrait&per_page=6');
        const urls = data.results.map(r => r.urls.raw + '&auto=format&fit=crop&q=80&w=200&h=200');

        // Fallbacks if Unsplash API fails or returns 0
        const fallbackUrls = [
            ...
            'https://images.unsplash.com/photo-1531384441138-2736e62e0919?auto=format&fit=crop&q=80&w=200&h=200',
            'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=200&h=200',
            'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200&h=200'
        ];

        const finalUrls = urls.length >= 5 ? urls : fallbackUrls;
...
\`;
    ...
    fs.writeFileSync('components/Testimonials.tsx', componentCode);
    console.log('Testimonials.tsx created with real portraits.');
  } catch (err) {
    console.error(err);
  }
}
